...
import {
  addMatch,
  updateMatch,
} from "./../../../redux/Matchs/matchs.actions";
...
export default function TableMatchs({ data }) {
  ...
  const handleCreate = () => {
    setForm(
      <MatchForm cancel={() => setShowModal(false)} confirm={handleAddSubmit} />
    );
    setShowModal((prev) => !prev);
  };
                ...
                data.map((match) => {
                  return (
                    <tr key={match.id} className="text-center">
                      <td onClick={() => handleEdit(match)} className="p-2 whitespace-nowrap">
                        <p className="text-green-700 text-center table-link">{match.userId}</p>
                      </td>
                      <td className="p-2 max-w-md whitespace-nowrap">
                          <p className="font-medium truncate">{match.userMatchId}</p>
                      </td>
                      <td className="p-2 whitespace-nowrap">
                        <div className="font-medium text-gray-800">
                          {match.matched ? <p className="text-green-400">SI</p>:<p className="text-red-400">NO</p>}
                        </div>
                      </td>
                      <td className="p-2 whitespace-nowrap">
                        <div className="font-medium text-gray-800">
