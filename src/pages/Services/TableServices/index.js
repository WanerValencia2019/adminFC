...
function TableUsers({ data }) {
    ...
    const renderEditModal = (user) => (
        <ServicesForm data={user} cancel={() => setShowModal(false)} confirm={handleEditSubmit} />
    );
    ...
    const handleCreate = () => {
        setForm(<ServicesForm cancel={() => setShowModal(false)} confirm={handleAddSubmit} />);
        setShowModal((prev) => !prev);
    };
                                ...
                                data.map((services) => {
                                    return (
                                        <tr key={services.id}>
                                            ...
                                            >
                                                <p className="text-green-700 text-left table-link">
                                                    {services.name}
                                                </p>
                                            </td>
